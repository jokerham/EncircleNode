...
const MemberList: React.FC = () => {
  ...
  const fetchUserPermissions = async (userId: string) => {
    try {
      ...
      const userWithPermissions = await userApi.getUserWithPermissions(userId);
      
      // Update the modal state with the fetched permissions
      setPermissionsModal(prev => ({
        ...prev,
        role: (userWithPermissions as User).roleId,
      }));
    } catch (err: unknown) {
      setPermissionsError((err as Error).message || 'Failed to load permissions');
  ...
  const handleRoleClick = async (user: User) => {
    // Open modal with basic role info first
    setPermissionsModal({
      open: true,
      role: user.roleId,
      userName: user.name,
